...
const listApplications = async (req, // The request coming from the client
...
const createApplication = async (req, // The request coming from the client
res // The response we will send back to the client
) => {
    try {
        const { 
        // Get all necessary data from the request body
        applicationDate, status, propertyId, tenantCognitoId, name, email, phoneNumber, message } = req.body;
        ...
        const newApplication = await prisma.$transaction(async (prisma) => {
            ...
            const lease = await prisma.lease.create({
                data: {
                    ...
                    endDate: new Date(new Date().setFullYear(new Date().getFullYear() + 1)), // Lease ends 1 year from today
...
const updateApplicationStatus = async (req, // The request from the client
res // The response we will send back to the client
) => {
    try {
